...
export default function HealthCalculator() {
    ...
    const downloadReport = async () => {
        ...
        const priorityHtml = renderPriorityActions().map(li =>
            `<li><strong>${li.props.children[0].props.children}</strong>: ${li.props.children[1]}</li>`
        ).join('');

        const planHtml = renderImprovementPlan().map((card, index) => `
        <div style="margin: 15px 0; padding: 15px; border: 1px solid #ddd; border-radius: 8px;">
            <h3>${card.props.children[0].props.children}</h3>
            <p><strong>Focus Area:</strong> ${card.props.children[1].props.children[1]}</p>
            <p><span style="padding:4px 12px; border-radius:999px; background:#dcfce7; color:#166534; font-weight:700;">
                ${card.props.children[2].props.children[1].props.children}
            </span></p>
            <ol>
                ${card.props.children[3].props.children.map((li, i) => `<li>${li.props.children}</li>`).join('')}
            </ol>
        </div>
    `).join('');
...
<head>
    ...
    <style>
        ...
        h1, h2 { color: #166534; }
        .band { padding: 4px 12px; border-radius: 999px; font-weight: 700; background: #dcfce7; color: #166534; }
